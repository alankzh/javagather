...
public class AAAJedisConnectionFactory implements InitializingBean, DisposableBean, RedisConnectionFactory{
     ...
     * shard information).
     */
    public AAAJedisConnectionFactory() {}
     ...
     */
    public AAAJedisConnectionFactory(JedisShardInfo shardInfo) {
     ...
     */
    public AAAJedisConnectionFactory(JedisPoolConfig poolConfig) {
     ...
     * @since 1.4
     */
    public AAAJedisConnectionFactory(RedisSentinelConfiguration sentinelConfig) {
     ...
     * @since 1.4
     */
    public AAAJedisConnectionFactory(RedisSentinelConfiguration sentinelConfig, JedisPoolConfig poolConfig) {
     ...
     * @since 1.7
     */
    public AAAJedisConnectionFactory(RedisClusterConfiguration clusterConfig) {
     ...
     * @since 1.7
     */
    public AAAJedisConnectionFactory(RedisClusterConfiguration clusterConfig, JedisPoolConfig poolConfig) {
        this.clusterConfig = clusterConfig;
    ...
    // todo 改造点1
    private static final String JedisClusterTopologyProviderClassName = "org.springframework.data.redis.connection.jedis.JedisClusterConnection$JedisClusterTopologyProvider";
    private static final String JedisClusterNodeResourceProviderClassName = "org.springframework.data.redis.connection.jedis.JedisClusterConnection$JedisClusterNodeResourceProvider";
    private JedisCluster createCluster() {
        JedisCluster cluster = createCluster(this.clusterConfig, this.poolConfig);
        try {
            Class topology = Class.forName(JedisClusterTopologyProviderClassName);
            Constructor topologyConstructor = topology.getDeclaredConstructor(JedisCluster.class);
            topologyConstructor.setAccessible(true);

            Class nodeResult = Class.forName(JedisClusterNodeResourceProviderClassName);
            Constructor nodeResultConstructor = nodeResult.getDeclaredConstructor(JedisCluster.class);
            nodeResultConstructor.setAccessible(true);

            this.clusterCommandExecutor = new ClusterCommandExecutor((ClusterTopologyProvider)topologyConstructor.newInstance(cluster),
                    (ClusterNodeResourceProvider)nodeResultConstructor.newInstance(cluster), EXCEPTION_TRANSLATION);
